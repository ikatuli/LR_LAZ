...
void fun_vref (){
	...
	for (y=0;y<gdk_pixbuf_get_height(pixbuf);y++){
		...
		put_pixel(pixbuf2,(gdk_pixbuf_get_width(pixbuf)-1)-x,y,tmp[0],tmp[1],tmp[2]);
		}
	}
	GooCanvasItem *ConvaImage2 = goo_canvas_image_new (root2,pixbuf2,0.0,0.0,NULL);
...
void fun_href (){
	...
	for (y=0;y<gdk_pixbuf_get_height(pixbuf);y++){
		for (x=0;x<gdk_pixbuf_get_width (pixbuf);x++){
		guchar *tmp=get_pixel(pixbuf,x,y);
		put_pixel(pixbuf2,x,(gdk_pixbuf_get_height(pixbuf)-1)-y,tmp[0],tmp[1],tmp[2]);
		}
	}
	GooCanvasItem *ConvaImage2 = goo_canvas_image_new (root2,pixbuf2,0.0,0.0,NULL);
...
void fun_lhalf (){
	...
	int row=gdk_pixbuf_get_rowstride (pixbuf),x,y;
	for (y=0;y<gdk_pixbuf_get_height(pixbuf);y++){
		for (x=0;x<(gdk_pixbuf_get_width (pixbuf)/2);x++){
		guchar *tmp=get_pixel(pixbuf,x,y);
		put_pixel(pixbuf2,x,y,tmp[0],tmp[1],tmp[2]);
		put_pixel(pixbuf2,x+(gdk_pixbuf_get_width (pixbuf)/2),y,tmp[0],tmp[1],tmp[2]);
		}
	}
	GooCanvasItem *ConvaImage2 = goo_canvas_image_new (root2,pixbuf2,0.0,0.0,NULL);
...
void fun_rhalf (){
	...
	int row=gdk_pixbuf_get_rowstride (pixbuf),x,y;
	for (y=0;y<gdk_pixbuf_get_height(pixbuf);y++){
		for (x=(gdk_pixbuf_get_width (pixbuf)/2);x<gdk_pixbuf_get_width (pixbuf);x++){
		guchar *tmp=get_pixel(pixbuf,x,y);
		put_pixel(pixbuf2,x,y,tmp[0],tmp[1],tmp[2]);
		put_pixel(pixbuf2,(x-(gdk_pixbuf_get_width (pixbuf)/2)),y,tmp[0],tmp[1],tmp[2]);
		}
...
//Кнопка 
	...
	//
	 ...
	 Vref =gtk_radio_button_new_with_label_from_widget (GTK_RADIO_BUTTON(CleanConva), "Вертикальное отражение");
	 Href =gtk_radio_button_new_with_label_from_widget (GTK_RADIO_BUTTON(CleanConva), "Горизонтальное отражение");
	 Lhalf=gtk_radio_button_new_with_label_from_widget (GTK_RADIO_BUTTON(CleanConva), "Левая половина");
	 Rhalf=gtk_radio_button_new_with_label_from_widget (GTK_RADIO_BUTTON(CleanConva), "Правая половина");
	 Uhalf=gtk_radio_button_new_with_label_from_widget (GTK_RADIO_BUTTON(CleanConva), "Отражение правой половины");
	 Bhalf=gtk_radio_button_new_with_label_from_widget (GTK_RADIO_BUTTON(CleanConva), "Отражение левой половины");
	 ...
	 gtk_box_pack_start(GTK_BOX(HboxRBatton),Copy,FALSE,FALSE,10);
